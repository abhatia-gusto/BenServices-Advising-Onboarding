-- email_recency: per-opp last advising OUTBOUND / advising-attributed INBOUND email date.
--
...
-- last_inbound_email_date  = latest advising-ATTRIBUTED inbound (same v10 ladder as above).
-- last_outbound_email_date = latest advising-OWNED outbound (sender subteam, TEAM_RENAME'd,
--   = 'Benefits Advising'). Both keyed to the opp via coalesce(tp.sfdc_opportunity_id,
--   c.sfdc_opportunity_id) — the canonical touchpoint->opp path — NOT the lossy BO_OPP_ID join
--   the previous version used (which returned NULL for opps like Amtech despite live TPs).
-- Drives the "we've gone quiet" / "customer gone quiet" risk signals. Feeds:
-- last_outbound_email_date, last_inbound_email_date. Grain: one row per opp. No PII (dates only).
with ee_out as (
    select
        ee.name as ee_name
        , ee.sfdc_user_id
...
, inbound_rec as (
  select a.sfdc_opportunity_id as opp, max(a.touchpoint_start_ts::date) as d
  from (
select z.*,
...
-- PARAMETERS
-- ──────────
--   2026-01-01  Date   Start of reporting window (MT)
--   2026-09-30    Date   End of reporting window (MT)
...
inbound as (
    ...
    where
        ...
        and coalesce(tp.sfdc_opportunity_id, c.sfdc_opportunity_id) in (
            select sfdc_object_id from bi_reporting.advising_opportunities
...
select
    ...
    , opp_owner_ee.pe       as opp_owner_pe
    , opp_owner_at_tp_ee.ee_name  as opp_owner_name_at_tp
    , coalesce(opp_owner_at_tp_hist.sub_team, opp_owner_at_tp_ee.sub_team) as opp_owner_subteam_at_tp
    , opp_owner_at_tp_ee.pe  as opp_owner_pe_at_tp
    , bo_owner.ee_name  as current_benefit_order_owner_name
    , bo_owner.sub_team as bo_owner_subteam
    , bo_owner.pe       as bo_owner_pe
    , bo_owner_at_tp_ee.ee_name  as bo_owner_name_at_tp
    , coalesce(bo_owner_at_tp_hist.sub_team, bo_owner_at_tp_ee.sub_team) as bo_owner_subteam_at_tp
    , bo_owner_at_tp_ee.pe  as bo_owner_pe_at_tp
...
left join ee_hist bo_owner_at_tp_hist
    on bo_owner_at_tp_hist.sfdc_user_id = boh.sfdc_stage_owner_id
    and convert_timezone('UTC','America/Denver', i.touchpoint_start_ts)::date >= bo_owner_at_tp_hist.effect_start_dt
    and convert_timezone('UTC','America/Denver', i.touchpoint_start_ts)::date <= bo_owner_at_tp_hist.effect_end_dt
...
left join ee_hist opp_owner_at_tp_hist
    on opp_owner_at_tp_hist.sfdc_user_id = oh.owner
    and convert_timezone('UTC','America/Denver', i.touchpoint_start_ts)::date >= opp_owner_at_tp_hist.effect_start_dt
    and convert_timezone('UTC','America/Denver', i.touchpoint_start_ts)::date <= opp_owner_at_tp_hist.effect_end_dt
where
    (
  ...
  where a.attributed_subteam = 'Benefits Advising'
    and a.sfdc_opportunity_id is not null
    and a.sfdc_opportunity_id in (select sfdc_object_id from bi_reporting.advising_opportunities
                                  where renewal_date in ({{cohort_dates}}))
  group by 1
)
, outbound_rec as (
  select coalesce(tp.sfdc_opportunity_id, c.sfdc_opportunity_id) as opp,
         max(tp.touchpoint_start_ts::date) as d
  from bi.cases c
  join bi.benefit_order_touchpoints tp on tp.sfdc_case_id = c.id
  join ee_out ei on ei.sfdc_user_id = tp.sfdc_advocate_user_id
  where tp.channel = 'Email' and tp.direction = 'Outbound' and tp.carrier_case_flag = false
    and c.type <> 'Carrier Submission'
    and (case ei.sub_team
        when 'Onboarding Advocacy' then 'New Plan & Renewal Onboarding'
        when 'Customer Advising' then 'Benefits Advising'
        when 'Group Operations Fulfillment' then 'Group Fulfillment'
        when 'Premier Dedicated Care' then 'Premier Dedicated Service Advising'
        when 'Premier Care' then 'Premier Dedicated Service Advising'
        else ei.sub_team end) = 'Benefits Advising'
    and coalesce(tp.sfdc_opportunity_id, c.sfdc_opportunity_id) in (select sfdc_object_id from bi_reporting.advising_opportunities
                                  where renewal_date in ({{cohort_dates}}))
  group by 1
)
select coalesce(i.opp, o.opp) as opp,
  to_char(o.d) as last_outbound_email_date,
  to_char(i.d) as last_inbound_email_date
from inbound_rec i
full outer join outbound_rec o on o.opp = i.opp
where coalesce(i.opp, o.opp) is not null
