...
-- PARAMETERS
-- ──────────
--   {{ TP Date Range Start }}  Date   Start of reporting window (MT)
--   {{ TP Date Range End }}    Date   End of reporting window (MT)
...
inbound as (
    ...
    where
        ...
        and tp.touchpoint_start_ts >=
            convert_timezone('America/Denver', 'UTC', '{{ TP Date Range Start }}'::timestamp_ntz)
        and tp.touchpoint_start_ts <
            convert_timezone('America/Denver', 'UTC', ('{{ TP Date Range End }}'::date + 1)::timestamp_ntz)
),
outbound as (
    select
        tp.sfdc_case_id
    ...
    where
        ...
        and tp.touchpoint_start_ts >=
            convert_timezone('America/Denver', 'UTC', '{{ TP Date Range Start }}'::timestamp_ntz)
        and tp.touchpoint_start_ts <
            convert_timezone('America/Denver', 'UTC', ('{{ TP Date Range End }}'::date + 8)::timestamp_ntz)
),
fallback_reply as (
    select
