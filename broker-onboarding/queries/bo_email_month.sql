SELECT IC,
  DATE_TRUNC('month',TPC)::date AS MONTH,
  SUM(IFF(ST IN ('Responded (Within SLA)','Responded (Past SLA)'),1,0)) AS ANSWERED,
  SUM(IFF(ST='Responded (Within SLA)',1,0)) AS WITHIN_SLA
FROM (
  SELECT
    CASE
...
--   2026-05-31    Date   End of reporting window (MT)
...
inbound as (
    ...
    where
        ...
        and tp.touchpoint_start_ts <
            convert_timezone('America/Denver', 'UTC', ('{{PERF_END}}'::date + 1)::timestamp_ntz)
),
outbound as (
    select
        tp.sfdc_case_id
    ...
    where
        ...
        and tp.touchpoint_start_ts >=
            convert_timezone('America/Denver', 'UTC', '2026-01-01'::timestamp_ntz)
        and tp.touchpoint_start_ts <
            convert_timezone('America/Denver', 'UTC', ('{{PERF_END}}'::date + 8)::timestamp_ntz)
),
fallback_reply as (
    select
...
where
  ...
  WHERE response_required_flag = TRUE
    ...
    AND tp_created_ts BETWEEN '2026-01-01' AND '{{PERF_END}} 23:59:59'
) r
WHERE RSUB IN ('Bring Your Broker','Benefits Transfers') AND IC IS NOT NULL
